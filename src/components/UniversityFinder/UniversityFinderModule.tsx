...
const PAK_LOGO_MAP: Record<string, string> = {
  ...
  "Quaid-i-Azam University": "/logos/Quaid-e-Azam University.png",
...
function createVirtualUniversity(
  ...
  programs: string[],
  options?: {
    country?: "Pakistan" | "United Kingdom" | string;
  ...
  if (feeStructureText) {
    tuitionFees.push({ program: "Programs", localFee: feeStructureText });
  } else if (minFee != null) {
...
export function UniversityFinderModule({ onBack }: UniversityFinderModuleProps) {
  ...
  const filteredUniversities = useMemo(() => {
    ...
    if (!hasActiveFilters) {
      combinedInstitutes.forEach((instituteName) => {
    ...
    return results;
  }, [filters, pakistaniUniversities, instituteDataMap, dbInstitutesToShow, combinedInstitutes]);
